	...
	with open("pmtdb.txt", "r") as f:
		...
		for i in range(0,len(data)):
			...
			if float(data[i][5])>.2 or float(data[i][5])<.07:
				continue
			if float(data[i][9])>40 or float(data[i][9])<22:
				continue
...
figpde=plt.figure(1,figsize=(10,6))
ax1=figpde.add_subplot(111)
pmtsns=['EA0339','EA0419','EA1578','EA1437','EA0574','EA0586','PA1705-117','PA1704-731','PA1704-108','PA1705-12']
#pmtsns=['EA0339','EA0419','EA1578','EA1437','EA0574','EA0586']
#pmtsns=['PA1705-117','PA1704-731','PA1704-108','PA1705-12']
for sn in pmtsns:
	time,arraypde=drawrefpmt(sn)
	plt.plot(time,arraypde)
plt.grid(ls=":",c='k')
plt.title(r'PDE of reference PMTs[%]')
plt.xlabel('time',fontsize=12)
plt.ylabel(r'PDE of reference PMTs[%]',fontsize=12)
#ax1.legend([r'$\mu$@setpoint3000[normalized to latest value]',r'$\mu$@0.1pe[normalized to latest value]','PDE@setpoint3000[normalized to latest value]'])
ax1.legend(pmtsns)
plt.savefig("pde_all_refPMTs"+".png")
